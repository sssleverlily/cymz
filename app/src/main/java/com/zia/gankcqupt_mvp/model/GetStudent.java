...
import com.zia.gankcqupt_mvp.utils.HttpUtil;
import com.zia.gankcqupt_mvp.utils.RegexUtil;
    ...
    public void GetFromCQUPT(final OnAllStudentGet onAllStudentGet, final GetProgress getProgress) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                String baseURL = "http://jwzx.cqupt.congm.in/jwzxtmp/";
                String html = null;
                try {
                    html = RegexUtil.getHtml(baseURL + "pubBjsearch.php?action=bjStu");
                } catch (IOException e) {
                    e.printStackTrace();
                    onAllStudentGet.onError();
                }
                Log.e(TAG, html);
                List<RegexUtil.Tag> classURLs = RegexUtil.getTags("a", html);
                classURLs.remove(0);
                int size = classURLs.size();
                i = 0;
                for (RegexUtil.Tag tag : classURLs) {
                    String studentHtml = null;
                    try {
                        studentHtml = RegexUtil.getHtml(baseURL + tag.getValue("href"));
                    } catch (IOException e) {
                        e.printStackTrace();
                        onAllStudentGet.onError();
                    }
                    i++;//当数字改变时才刷新进度，节省内存
                    if (p != (int) ((float) i * 100f / (float) size)) {
                        p = (int) ((float) i * 100f / (float) size);
                        Log.d(TAG, "当前进度： " + p);
                        getProgress.status(p);
                    }
                    List<String> tds = RegexUtil.regexExcept("<tr>", "</tr>", studentHtml);
                    tds.remove(0);
                    for (String trHtml : tds) {
                        List<String> trs = RegexUtil.regexExcept("<td>", "</td>", trHtml);
                        Student student = new Student();
                        student.setClassnum(trs.get(0));
                        student.setStudentid(trs.get(1));
                        student.setName(trs.get(2));
                        student.setSex(trs.get(3));
                        student.setClassid(trs.get(4));
                        student.setZyh(trs.get(5));
                        student.setMajor(trs.get(6));
                        student.setCollege(trs.get(7));
                        student.setYear(trs.get(8));
                        student.setAtschool(trs.get(9));
